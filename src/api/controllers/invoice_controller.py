...
from api.decorators.auth_decorators import require_permission, login_required
from flask import g

# Initialize services
invoice_service = InvoiceService(InvoiceRepository(), UserRepository())
...
@require_permission(function_code="F111", methods=["GET"])
...
@require_permission(function_code="F111", methods=["POST"])
def create_invoice():
    """
    ...
    invoice = invoice_service.create_invoice(
        ...
        vat_total=data.get('vat_total', 0.0),
...
@require_permission(function_code="F111", methods=["PUT"])
...
@require_permission(function_code="F111", methods=["DELETE"])
    ...
    invoice_dict['details'] = [
        {
            ...
            'vat': getattr(d, 'vat', 0.0),
            'discount': getattr(d, 'discount', 0.0),
...
@require_permission(function_code="F207", methods=["POST"])
...
@require_permission(function_code="F209", methods=["GET"])
            ...
            application/json:
              schema:
                ...
                items:
                  $ref: '#/components/schemas/InvoiceResponse'
    """
    invoices = invoice_service.list_user_invoices(g.user_id)
    return jsonify(response_schema.dump(invoices, many=True)), 200
...
def get_employee_invoice(invoice_id):
    ...
    # Verify ownership
    if str(invoice.created_by) != str(g.user_id):
...
@require_permission(function_code="F208", methods=["PUT"])
...
@require_permission(function_code="F210", methods=["PUT"])
...
def get_employee_invoice_details(invoice_id):
    ...
    # Verify ownership
    if str(invoice.created_by) != str(g.user_id):
        return jsonify({'error': 'Access denied: You can only view details of your own invoices'}), 403
...
@require_permission(function_code="F213", methods=["GET"])
...
@require_permission(function_code="F214", methods=["PUT"])
