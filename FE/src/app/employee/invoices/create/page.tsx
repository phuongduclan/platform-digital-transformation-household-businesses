...
import { OwnerProduct, OwnerUnit, OwnerCustomer } from "@/services/owner.service";
...
export default function EmployeeCreateInvoicePage() {
  ...
  const fetchMeta = async () => {
    try {
      const [prods, uns, custs] = await Promise.all([
        employeeService.listProducts({ status: "Active" }),
        employeeService.listUnits({ status: "Active" }),
        employeeService.listCustomers({ status: "Active" }),
      ]);
      setProducts(prods);
      setUnits(uns);
      setCustomers(custs);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSaving(true);
      const payload = {
        seller_id: sellerId === "" ? undefined : Number(sellerId) || null,
        customer_id: customerId === "" ? undefined : Number(customerId) || null,
        invoice_type: invoiceType,
        description: description.trim() || undefined,
        // status không gửi, BE sẽ default Draft cho Employee
        details: lines.map((l) => ({
      ...
      const created: EmployeeInvoice = await employeeService.createInvoice(
        payload
      );
  ...
  return (
    <>
      <LoadingOverlay isLoading={saving} message="Đang lưu hóa đơn..." />
      <div className="p-8">
        {/* Breadcrumb */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-lg shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Người bán: Employee không có quyền xem danh mục nhà cung cấp (F110) nên ẩn dropdown.
                  Nếu cần dùng sau này, sẽ tạo endpoint riêng cho Employee. */}
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const next = val === "" ? "" : (Number(val) as number | "");
                    setCustomerId(next);
                    if (next === "") {
                      setInvoiceType("PAID");
                    }
