...
export default function EmployeeInventoryPage() {
  const [inventory, setInventory] = useState<EmployeeInventory[]>([]);
  ...
  const handleRowClick = async (row: EmployeeInventory) => {
    try {
      const invDetail = await employeeService.getInventoryDetail(
        row.product_id,
        row.warehouse_id,
        { unit_id: row.unit_id }
      );
  ...
  return (
    <>
      <LoadingOverlay isLoading={loading} message="Đang tải tồn kho..." />
      <div className="p-8">
              ...
              <div className="space-y-3 text-sm">
                <div>
                  ...
                  <p className="text-base font-bold text-[#1e3a8a]">
                    {detail.product?.name ||
                      `SP #${detail.inventory.product_id}`}
                  </p>
                </div>
                <div>
