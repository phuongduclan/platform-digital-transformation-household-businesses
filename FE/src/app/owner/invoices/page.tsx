...
export default function OwnerInvoicesPage() {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      const data = await ownerService.listInvoicesOwner();
      setInvoices(data);
    } catch (error: any) {
      showToast(
  ...
  const handleDelete = async (invoice: OwnerInvoice) => {
    if (!confirm("Bạn có chắc chắn muốn xóa hóa đơn này?")) return;
    try {
      await ownerService.deleteInvoice(invoice.id);
  ...
  const handleConfirm = async (invoice: OwnerInvoice) => {
    ...
    <>
      ...
      <div className="p-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => router.push("/owner/invoices/create")}
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() =>
                              router.push(`/owner/invoices/${inv.id}`)
                            }
