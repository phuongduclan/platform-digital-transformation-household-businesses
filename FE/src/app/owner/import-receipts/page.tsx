...
export default function OwnerImportReceiptsPage() {
  const router = useRouter();
  const [receipts, setReceipts] = useState<OwnerImportReceipt[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [rs, whs] = await Promise.all([
        ownerService.listImportReceipts(),
                    ...
                    <tr key={r.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-3">
                        <button
                          type="button"
                          onClick={() =>
                            router.push(`/owner/import-receipts/${r.id}`)
                          }
